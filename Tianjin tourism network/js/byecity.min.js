...
Byecity.Common.Extend = {
    /*
    参数说明: tabID-外面大容器的ID,各个tab的样式名称
    样式：必须对应相应的样式文件
    调用方式如：
    Tab: Byecity.Common.Extend.CreateLiTab("byecity_tag")
    PinYin:Byecity.Common.Extend.CC2PY("例如")
    */
    CreateLiTab: function(tabID, contentID) {
        var list = $("#" + tabID + ">ul>li");
        list.each(function(i) {
            this.sign = i;
        });
        var p = $("#" + contentID + ">div[tag='TabDiv']");
        list.click(function() {
            if (this.className == 'hover') return;
            if (!p[this.sign]) {
                alert('sorry,we do not have this tag!');
                return;
            }
            list.removeClass('hover');
            p.css("display", "none");
            p[this.sign].style.display = "block";
            this.className = 'hover';
        });
        //设置合作伙伴的页面高度自适应 njz 20120328
        if ($.cookie("Byecity_utm_source") != null && $.cookie("Byecity_utm_source") == "byecityunion" && $.cookie("WebAffiliate") != null) {
            $.ajax({
                type: "GET",
                url: "Ajax/UserHandler.ashx",
                data: "method=cooperatechannel",
                async: false,
                success: function(data) {
                    switch (data) {
                        ...
                        case "-2":
                            break;
                        default:
                            if (data.length > 0 && data.indexOf("http") > -1) {
                                DoWithCooperaterURL(data);
                            }
                            break;
                    }
                }
...
//扩展一个Byecity.Common.GetXmlDom类
Byecity.Common.GetXmlDom = function() { };

Byecity.Common.GetXmlDom.prototype.xmlDom = function() {
    if (window.ActiveXObject) {
        var arrSignatures = ["MSXML2.DOMDocument.5.0", "MSXML2.DOMDocument.5.0", "MSXML2.DOMDocument.4.0",
...
//会员监测
window._ozuid = $.cookie("currentUserId");
...
$(document).ready(function() {
    ...
    var $divCuXiaoMa = $("#divCuXiaoMa");
    var $spanCuXiaoMa = $("#spanCuXiaoMa");
    var $divCuXiaoMa_layer = $("#divCuXiaoMa_layer");
    if ($.url().param("utm_source") == "bd" || $.url().param("utm_source") == "gg" || $.cookie("Byecity_utm_source") == "bd" || $.cookie("Byecity_utm_source") == "gg") {
        $divCuXiaoMa.show();
        var randomNum = Math.round(Math.random() * 1000000);
        $divCuXiaoMa.attr('onmousemove', 'setClickTypeForLog(3,' + randomNum + ')');
        $spanCuXiaoMa.text("促销码：{0}".format(randomNum));


        $divCuXiaoMa.hover(function() {
            $divCuXiaoMa_layer.show();
        }, function() {
            $divCuXiaoMa_layer.hide();
        });
    }
    //为静态页面加载头尾\或者根据参数showhf hidhf hidall为非动态面处理页面头尾
    if ((document.URL.indexOf(".aspx") == -1 && document.URL.indexOf(".html") == -1 && document.URL.indexOf(".ashx") == -1 && document.URL.indexOf(".asmx") == -1) || (document.URL.indexOf("go.byecity.com") > 0 && document.URL.indexOf(".html") > 0)) {
        var showhf = "utf.htm"; //默认显示头尾的文件名
        var hidhf = "0"; //默认显示
        if (!($.url().param("showhf") == undefined) && $.url().param("showhf") != null && $.url().param("showhf").length > 0) {
            showhf = $.url().param("showhf");
        }
        else if ($("#hidshowhf").val() != undefined && $("#hidshowhf").val().trim().length > 0) {
            showhf = $("#hidshowhf").val();
        }

        if (!($.url().param("hidhf") == undefined) && $.url().param("hidhf") != null && $.url().param("hidhf").length > 0) {
            hidhf = $.url().param("hidhf");
        }
        else if ($("#hidhf").val() != undefined && $("#hidhf").val().trim().length > 0) {
            hidhf = $("#hidhf").val();
        }

        if ($("#jsTop").length == 1) {
            if (hidhf != 1 && ($("#jsTop").html() == undefined || $("#jsTop").html().indexOf("<") == -1) && document.URL.indexOf("http://go.byecity.com") == -1) {
                $.ajax({
                    type: "GET",
                    url: "/Ajax/UserHandler.ashx",
        ...
        if ($("#jsBottom").length == 1) {
            if (hidhf != 1 && ($("#jsBottom").html() == undefined || $("#jsBottom").html().indexOf("<") == -1) && document.URL.indexOf("http://go.byecity.com") == -1) {
                $.ajax({
                    type: "GET",
                    url: "/Ajax/UserHandler.ashx",
        ...
        //根据相关参数再次处理头尾[服务器对此参数的相关逻辑已做处理]
        if (hidhf == 1) {
            if (showhf == "") {//liuqing 2013-4-16
                $("#jsTop").hide();
                $("#jsBottom").hide();
            }
        }
    ...
    if ($.url().param("showhf") && !($.url().param("showhf") == undefined)) {
        ...
        //liuqing 2013-04-16
        if ($.url().param("showhf") == "360lv.htm") {
            $("body").append("<iframe id=\"container-frame\" scrolling=\"no\" style=\"display:none;\" name=\"container-frame\" src=\"http://union.byecity.com/Transfer.aspx?UserID=143162&AdID=93&SiteID=103&utm_source=byecityunion&unionorderchanel=422\" style=\"display:none;\" ></iframe>");

            $("#top_nav_visa_country a").each(function() {
                if ($(this).attr("href") != undefined) {
                    $(this).attr("target", "_parent");

                }
            });
            $(".vd_table a").each(function() {
                if ($(this).attr("href") != undefined) {
                    $(this).attr("target", "_parent");
            ...
            success: function(data) {
                switch (data) {
                    ...
                    default:
                        if (data.length > 0 && data.indexOf("http") > -1) {
                            DoWithCooperaterURL(data);
                            //处理go.byecity.com下面的文章链接
                            if ($.url().param("hidall") && !($.url().param("hidall") == undefined)) {
                                var re = /guide/g;
                                $("a[href^='http://go.byecity.com']").each(function() {
                                    var href = DoWithHideAll($(this).attr("href"));
                                    if (href.indexOf("http://go.byecity.com/guide") > -1 && $.url(href).attr("path").endWith("html")) {

                                        href = href.replace(re, "Cooperateguide");
                                    }
                                    $(this).attr("href", href);
                                    $(this).attr("target", "_self");
                                });
    ...
    if (document.URL.indexOf("http://log.byecity.com") == -1 && document.URL.indexOf("5.byecity.com") == -1) {
        var baseid = 0;
        var cid = 0;
        if (document.getElementById("BaseID"))
            baseid = document.getElementById("BaseID").value;
        if ($.url().param("utm_term") && $.url().param("utm_term") != undefined)
            cid = $.url().param("utm_term");
        $("body").append("<iframe id=\"{0}\" scrolling=\"no\"  name=\"{0}\" src=\"http://log.byecity.com/customerrecord.aspx?currentURL={1}&productID={2}&lastURL={3}&cid={4}\" style=\"display:none;\" ></iframe>".format("IframeCustomerRecord", encodeURIComponent(document.URL), baseid, encodeURIComponent(document.referrer), cid));
        //99click 监测代码 只能加载一次，如果是跟踪日志的页面就不再加载

        $("body").append("<script language=\"JavaScript\" src=\"http://www.byecity.com/js/99click.js\"></script>");
        if ($("#jsTop").length == 1) {
            $("body").append("<script language=\"JavaScript\" src=\"http://www.byecity.com/js/plugins/autocomplete.min.js\"></script>");
            $("body").append("<script language=\"JavaScript\" src=\"http://www.byecity.com/js/top_search_Common.js\"></script>");
        }
        if ($("span>a.white").length > 0)
            $("span>a.white").attr('rel', 'nofollow');

    }
    //公告关闭效果
    $("#site_layer_close").click(function() {
        $("#visa_tg_layer").css("display", "none");
    }), $("#visa_tg em").hover(function() {
        $("#visa_tg_layer").fadeIn(600);
    })
});
...
//设置合作伙伴的页面高度自适应
function DoWithCooperaterURL(cooperaterURL) {
    var hashH = document.body.scrollHeight;
    hashH = hashH + 30;
    cooperaterURL = "{0}?t={2}#height={1}".format(cooperaterURL, hashH, Math.random()); //将高度作为参数传递
    $("body").append("<iframe id=\"{0}\" name=\"{0}\" src=\"{1}\" style=\"display:none;\" ></iframe>".format("iframeA", cooperaterURL));
    
}
//根据某一元素设置合作伙伴的嵌套页面高度
function ReDoWithCooperaterURL(cooperaterURL, ElementID, offsetHeight) {
    var Height_PianYi = 0;
    if (!isNaN(offsetHeight)) {
        Height_PianYi = Math.round(offsetHeight);
    }
    var hashH = getElementPos(ElementID).y + Height_PianYi; //document.documentElement.clientHeight;
...
function DoWithShowhf(href_old_string) {
    //如果href中含有javascript 则不做任何处理直接返回 njz20120220
    if (href_old_string != undefined && href_old_string.indexOf("javascript") != -1) {
        return href_old_string;
    }
    ...
    if (!($.url().param("showhf") == undefined)) {//判断页面URL中是否含showhf参数
        if (!href_old.param("showhf") && href_old.param("showhf") == undefined) {//判断参数href_old_string里是否含showhf参数
            ...
            else if (query && !(query == undefined) && query.indexOf("showhf") === -1) {
                href_new = "{0}&showhf={1}".format(href_old.attr("source"), $.url().param("showhf"));
            }
        }
    }

    //liuqing 2013-04-25 应对签证搜素按钮的点击拼接字符串
    if (!($.url().param("hidall") == undefined)) {
        if (!href_old.param("hidall") && href_old.param("hidall") == undefined) {
            href_new = href_new + "&hidhf=1&hidall=1";
...
function DoWithHideAll(href_old_string) {
    ...
    if (href_old_string == undefined) {
        return "";
    }
    //如果href中含有javascript 则不做任何处理直接返回 njz20120220
    if (href_old_string != undefined && (href_old_string.indexOf("javascript") != -1 || href_old_string.startWith("#"))) {
        return href_old_string;
    }
    if (href_old_string != undefined && href_old_string.endWith("#")) {
        href_old_string = href_old_string.substring(0, href_old_string.Length - 1);
    }
    var href_old = $.url(href_old_string, true); //转换成URL对象
    var href_new = href_old_string; //默认情况下新旧URL一样
    if (!($.url().param("hidall") == undefined)) {//判断页面URL中是否含hidall参数
        if ($.url().param("hidall") == 1) {
            //start 如果里面还有showhf liuqing 2013-04-16
            if (!($.url().param("showhf") == undefined)) {//判断页面URL中是否含showhf参数
                var query = href_old.attr("query");

                if (!href_old.param("hidall") && href_old.param("hidall") == undefined) {//判断参数href_old_string里是否含showhf参数
                   
                    if (query == undefined || href_old_string.indexOf("?") === -1) {
                ...
                if (!href_old.param("showhf") && href_old.param("showhf") == undefined) {

                    href_new = href_new + "&showhf="+ $.url().param("showhf");
                }
                ...
                if (!href_old.param("hidall") && href_old.param("hidall") == undefined) {//判断参数href_old_string里是否含hidall参数
                    ...
                    else if (!(query == undefined) && query.length > 0 && query.indexOf("hidall") === -1) {
                        href_new = "{0}&hidhf=1&hidall={1}".format(href_old.attr("source"), $.url().param("hidall"));
                    }
                }
            }
...
//酒店频道在用
function setTab(CurrentSelectedindex, Count, Menu_Public, Content_Public, selectedclassName) {
    for (i = 1; i <= Count; i++) {
...
//记录用户是否点击特殊元素 刘易 2012-7-9
function setClickTypeForLog(num, code) {
    var name = "Byecity_ClickType";
    var value = $.cookie(name);
    if (value == null || value == "") {
        value = num;
        if (typeof code != 'undefined')
            value += "|" + code;
        $.cookie(name, value, { expires: 1, path: '/', domain: 'byecity.com' });
    }
    else {
        var arr = value.split(',');
        var f = true;
        for (var i = 0; i < arr.length; i++) {
            if (arr[i] == num || arr[i] == num + "|" + code) {
                f = false;
                break;
            }
        }
